...
('pass000' , 'Aaron' ,'Green' , 'AaronGreen@gamil.com' ,'Achziv' , 'Entrance_emp'),
('pass111' , 'Bob' ,'Blue' , 'AaronGreen@gamil.com' ,'Ein Gedi' , 'Customer_service');
...
CREATE PROCEDURE PopulateGoNatureOrders()
BEGIN
    ...
    WHILE i <= 1000 DO
        SET v_order_date = DATE_ADD('2026-06-01', INTERVAL FLOOR(RAND() * 20) DAY);
        
        SET v_park = ELT(FLOOR(1 + RAND() * 5), 'Achziv', 'Banias', 'Caesarea', 'Ein Gedi', 'Masada');
        ...
        SET v_entry_time = SEC_TO_TIME(25200 + FLOOR(RAND() * 39600));
        SET v_status = ELT(FLOOR(1 + RAND() * 4), 'Entered', 'Confirmed', 'Canceled', 'On waiting list');
        ...
        SET v_id = FLOOR(100000000 + RAND() * 899999999);
...
CALL PopulateGoNatureOrders();
DROP PROCEDURE PopulateGoNatureOrders;