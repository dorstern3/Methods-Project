...
public class EchoServer extends AbstractServer {
	...
	/**
	 * Processes a park exit request sent from either a park employee or a
	 * standalone visitor gate. Maps identifications dynamically using the single
	 * 'id' column context. Updates order status to 'Completed' and releases park
	 * occupancy.
	 *
	 * @param message The message containing an Object array: [0] OrderID/QRCode
	 *                (String), [1] TravelerID (String or null).
	...
	private void handleExitPark(Message message, ConnectionToClient client) {
		...
		try {
			...
			// Step 1: Formulate the query based on the authorization level using the single
			// id column
			if (travelerId == null) {
				...
				// Standalone Visitor Mode: High security, cross-reference directly with the
				// single id field
				selectQuery = "SELECT order_number, number_of_visitors, park_name FROM gonature_db_new.`Order` "
			...
			if (rs.next()) {
				...
				// Step 3: Update departure time AND set status to 'Completed' to close the
				// cycle properly
	...
	private void handleValidateOrder(Message message, ConnectionToClient client) {
		...
		try {
			...
			String query = "SELECT number_of_visitors, type_of_visitor, order_date, entry_time, status "
					+ "FROM gonature_db_new.`Order` " + "WHERE (order_number = ? OR QR_code = ?) AND exit_time IS NULL";
	...
	private void handleVerifySubscriber(Message message, ConnectionToClient client) {
		...
		try {
			client.sendToClient(new Message(MessageType.VERIFY_SUBSCRIBER_RESPONSE, isSubValid));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * Processes transaction executions for visitor admissions inside the secure
	 * server framework. Increments designated park capacities dynamically and
	 * structures order history data logs. Evaluates casual vs pre-booked states,
	 * routing the client identification index to either the 'id' column or
	 * 'sub_number' column based on business rules.
	 *
	 * @param message The input payload wrapper holding the sequential transactional
	...
	/**
	 * Handles incoming login requests from clients. Validates credentials against
